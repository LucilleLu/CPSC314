...
void main() {
    ...
    vec3 T = cross(N, up);
    vec3 B = cross(N, T);
    mat3 TBNMatrix = mat3(T,B,N); // tangent space matrix of a vertex on a surface,
                                  // a matrix that converts normals in a normal map from tangent space to object space
