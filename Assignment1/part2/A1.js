...
var torusPosition = {type: 'v3', value: new THREE.Vector3(0,2,1)}
...
var torusMaterial = new THREE.ShaderMaterial({
	uniforms:{
...
// CREATE light
var lightbulbGeometry = new THREE.SphereGeometry(0.2, 32, 32);
var lightbulb = new THREE.Mesh(lightbulbGeometry, lightbulbMaterial);
lightbulb.parent = worldFrame;
scene.add(lightbulb);

// CREATE Torus
var torusGeometry = new THREE.TorusGeometry(0.5, 0.1, 20, 33);
...
function checkKeyboard() {
  ...
  //changing x,y,z position of the torus 
  if (keyboard.pressed("I"))
  	torusPosition.value.z -= 0.1;
  else if (keyboard.pressed("K"))
  	torusPosition.value.z += 0.1;
  
  if (keyboard.pressed("J"))
  	torusPosition.value.x -= 0.1;
  else if (keyboard.pressed("L"))
  	torusPosition.value.x += 0.1;

  if (keyboard.pressed("O"))
  	torusPosition.value.y -= 0.1;
  else if (keyboard.pressed("P"))
  	torusPosition.value.y += 0.1;

  lightbulbMaterial.needsUpdate = true;
  torusMaterial.needsUpdate = true; // Tells three.js that some uniforms might have changed
}
